...
-start mb-4"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard
              icon={<FaFileAlt className="h-5 w-5" />}
              title="Total Ujian"
              value={studentReportData.stats.totalExams}
              color="text-blue-600"
            />
            <StatCard
              icon={<FaChartBar className="h-5 w-5" />}
              title="Rata-rata Nilai"
              value={`${studentReportData.stats.averageScore.toFixed(1)}`}
              color={getScoreColor(studentReportData.stats.averageScore)}
            />
            <StatCard
              icon={<FaTrophy className="h-5 w-5" />}
              title="Nilai Tertinggi"
              value={studentReportData.stats.highestScore}
              color="text-green-600"
            />
            <StatCard
              icon={<FaExclamationTriangle className="h-5 w-5" />}
              title="Nilai Terendah"
              value={studentReportData.stats.lowestScore}
              color="text-red-600"
            />
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {studentReportData.stats.averageScore >= 85 && (
                    <div className="flex items-center text-sm text-green-600 dark:text-green-400">
                      <span className="w-2 h-2 bg-green-500 rounded-full mr-2"></span>
                      Performa sangat baik dengan rata-rata tinggi
                    </div>
                  )}
                  {studentReportData.stats.averageScore >= 70 &&
                    studentReportData.stats.averageScore < 85 && (
                      <div className=" dark:text-yellow-400 flex items-center text-sm text-yellow-600">
                        <span className="w-2 h-2 bg-yellow-500 rounded-full mr-2"></span>
                        Performa stabil dengan nilai memuaskan
                      </div>
                    )}
                  {studentReportData.stats.totalExams >= 5 && (
              ...
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {studentReportData.stats.lowestScore < 50 && (
                    <div className="flex items-center text-sm text-red-600 dark:text-red-400">
                      <span className="w-2 h-2 bg-red-500 rounded-full mr-2"></span>
                      Ada ujian dengan nilai sangat rendah
                    </div>
                  )}
                  {studentReportData.stats.totalExams < 3 && (
