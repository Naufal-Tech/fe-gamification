...
const ParentSidebar = ({ isCollapsed, toggleCollapse }) => {
  const navGroups = [
    {
      label: "Overview",
      links: [
    ...
    },
    {
      ...
      links: [
        {
          to: "/parents/tugas-reports",
          label: "Laporan Nilai Tugas",
          icon: <FaClipboardList className="h-5 w-5" />,
        },
        {
          to: "/parents/exam-reports",
          label: "Laporan Nilai Ujian",
  ...
  return (
    <div
      ...
      } bg-white dark:bg-gray-800 min-h-screen p-4 shadow-lg transition-all duration-200 flex flex-col sticky top-0`}
    >
          ...
          <div key={group.label} className="space-y-2">
            ...
            {group.links.map((link) => (
              <NavLink
                key={link.to}
                to={link.to}
                // 'end' prop ensures the dashboard link is only active when exactly on /parents/dashboard
                end={link.to === "/parents/dashboard"}
                className={({ isActive }) =>
