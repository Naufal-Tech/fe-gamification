...
const AdminTeacherEdit = () => {
  ...
  // Fixed the useState declaration
  const [file, setFile] = useState(null);
  const [previewImage, setPreviewImage] = useState("");

  // Set initial form data when teacher data loads
  useEffect(() => {
    if (teacherData) {
      setFormData({
        ...
        noIdentity: teacherData.noIdentity || "",
        isVerified: teacherData.isVerified || false,
        img_profile: teacherData.img_profile || "",
      });
      setPreviewImage(teacherData.img_profile || "");
    }
  }, [teacherData]);
  ...
  const updateTeacherMutation = useMutation({
    ...
    onError: (error) => {
      toast.error(
        error.response?.data?.error || "Failed to update teacher profile"
      );
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    updateTeacherMutation.mutate(formDataToSend);
  };
        ...
        <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 rounded-lg">
          <div className="flex items-center">
            ...
            <span className="text-sm sm:text-base">
              {error.response?.data?.error || "Failed to load teacher data"}
            </span>
          </div>
        </div>
        <Link
        ...
        <div className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end space-x-4 pt-6">
              <button
                type="button"
                onClick={() => navigate(`/admin/teachers/detail/${id}`)}
