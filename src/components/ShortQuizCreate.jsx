...
function ShortQuizCreate() {
  // Get data from loader
  const { categories, kelasList } = useLoaderData();
  ...
  useEffect(() => {
    console.log("Loader Data:", { categories, kelasList });
  ...
  const [formData, setFormData] = useState({
    ...
    description: "",
    category: "",
    kelas: [], // Changed to array to support multiple selection
    semester: "",
  ...
  // Mutation for creating quiz
  const createMutation = useMutation({
    mutationFn: (data) =>
      api.post(`/v1/short-quiz`, data, {
    ...
    onMutate: async (newData) => {
      ...
      queryClient.setQueryData(["shortQuiz"], (old) => {
        ...
        return {
          ...old,
          data: [{ ...newData, _id: "temp-id", status: "draft" }, ...old.data],
        };
      });
      return { previousList };
    },
    onError: (err, _, context) => {
      ...
      toast.error(err.response?.data?.error || "Failed to create short quiz", {
        duration: 6000,
        style: {
          background: "#ef4444",
          color: "#ffffff",
    ...
    onSuccess: (response) => {
      toast.success(
        response.data?.message || "Short quiz created successfully",
        {
          duration: 4000,
          style: {
            background: "#10b981",
...
export default ShortQuizCreate;
